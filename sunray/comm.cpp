...
void processWifiMqttClient()
{
  ...
  if (millis() >= nextMQTTPublishTime){
    ...
    if (mqttClient.connected()) {
      ...
      // statistics
      MQTT_PUBLISH(statIdleDuration, "%lu", "/stats/idleDuration")
      MQTT_PUBLISH(statChargeDuration, "%lu", "/stats/chargeDuration")
      MQTT_PUBLISH(statMowDuration, "%lu", "/stats/mow/totalDuration")
      MQTT_PUBLISH(statMowDurationInvalid, "%lu", "/stats/mow/invalidDuration")
      MQTT_PUBLISH(statMowDurationFloat, "%lu", "/stats/mow/floatDuration")
      MQTT_PUBLISH(statMowDurationFix, "%lu", "/stats/mow/fixDuration")
      MQTT_PUBLISH(statMowFloatToFixRecoveries, "%lu", "/stats/mow/floatToFixRecoveries")
      MQTT_PUBLISH(statMowObstacles, "%lu", "/stats/mow/obstacles")
      MQTT_PUBLISH(statMowGPSMotionTimeoutCounter, "%lu", "/stats/mow/gpsMotionTimeouts")
      MQTT_PUBLISH(statMowBumperCounter, "%lu", "/stats/mow/bumperEvents")
      MQTT_PUBLISH(statMowSonarCounter, "%lu", "/stats/mow/sonarEvents")
      MQTT_PUBLISH(statMowLiftCounter, "%lu", "/stats/mow/liftEvents")
      MQTT_PUBLISH(statMowMaxDgpsAge, "%.2f", "/stats/mow/maxDgpsAge")
      MQTT_PUBLISH(statMowDistanceTraveled, "%.1f", "/stats/mow/distanceTraveled")
      MQTT_PUBLISH(statMowInvalidRecoveries, "%lu", "/stats/mow/invalidRecoveries")
      MQTT_PUBLISH(statImuRecoveries, "%lu", "/stats/imuRecoveries")
      MQTT_PUBLISH(statGPSJumps, "%lu", "/stats/gpsJumps")      
